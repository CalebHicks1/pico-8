function _init()
	rand_seed = 4
	init_game()
end

function init_game()
	srand(rand_seed)
	cls()
	pen = new_pen()
	ground = {}
	camera_x = 0

	generate_ground()
	dt = 1 / 30
	ball = new_ball()
	-- add grav to ball
	-- acceleration due to grav
	grav = 100
	-- debug
	dx = 0
	dy = 0
	m_perp = vec()
	ground_force = vec()
end

function _update()
	dt = 1 / 30
	-- camera controls
	-- if btn(4) then
	-- 	camera_x -= 1
	-- end
	-- if btn(5) then
	-- 	camera_x += 1
	-- end
	-- slow motion
	if btn(4) then
		dt = 1 / 120
	end
	-- add gravity to ball
	add(ball.forces, vec(0, ball.m * grav))
	update_ball(ball)
	normal = normalize_vec({ x = dx, y = dy })
	if btnp(2) then
		rand_seed += 1
		init_game()
	end
end

function update_ball(b)
	-- sum forces on ball
	ax = 0
	ay = 0
	-- touching ground?
	if b.pos.y + (b.vel.y * dt) >= ground[flr(b.pos.x)] then
		-- the center of the ball is touching the ground
		--dt = 0
		momentum = vec(b.m * b.vel.x / dt, b.m * b.vel.y / dt)
		-- momentum_y = b.m * b.vel.y
		-- force_y = -2 * momentum_y / dt
		-- add(b.forces, vec(0, force_y))
		-- what is the slope at point of collision?
		dx = 2 * b.r
		dy = -1 * (ground[flr(b.pos.x - b.r)] - ground[flr(b.pos.x + b.r)])
		m = dy / dx
		m_vec = vec(dx, dy)
		m_norm = normalize_vec(m_vec)
		-- vector perpendicular to ground
		m_perp = vec(m_norm.y, -m_norm.x)
		momentum_magnitude = 50 * get_vec_length(momentum)
		ground_force = vec(momentum_magnitude * m_perp.x, momentum_magnitude * m_perp.y)
		add(b.forces, ground_force)
	end
	for f in all(b.forces) do
		ax += f.x / b.m
		ay += f.y / b.m
	end
	-- update ball position
	b.vel.x += ax * dt
	b.vel.y += ay * dt
	b.pos.x += b.vel.x * dt
	b.pos.y += b.vel.y * dt
	-- clear forces for next frame
	b.forces = {}
end

function _draw()
	cls()
	camera(camera_x, 0)
	draw_ground(camera_x)
	draw_ball(ball)
	-- debug lines
	-- draw the slope
	line(b.pos.x, b.pos.y, b.pos.x + dx, b.pos.y + dy, 3)
	-- perpendicular
	line(b.pos.x, b.pos.y, b.pos.x + dy, b.pos.y - dx, 12)
	camera()
	-- print(tostr(normal.x) .. "," .. tostr(normal.y), 10, 10)
	-- print(tostr(m_perp.x) .. "," .. tostr(m_perp.y), 10, 10)
	print(tostr(ground_force.x) .. "," .. tostr(ground_force.y), 10, 10)
	-- print(tostr(ball.pos.x) .. "," .. tostr(ball.pos.y), 10, 10)
end

function draw_ground(start_x)
	for x = max(0, start_x), start_x + 128 do
		if ground[x] == nil then
			break
		end
		line(x, ground[x], x, 128, 4)
		line(x, ground[x], x, ground[x] + 5, 9)
	end
end

function draw_ball(b)
	circ(b.pos.x, b.pos.y, b.r, 8)
end

-- constructors
function new_pen()
	p = {
		x = 0,
		dx = 1,
		y = 64,
		slope = 0.1,
	}
	return p
end

function new_ball()
	b = {
		pos = vec(64, 10),
		vel = vec(),
		m = 2,
		r = 5,
		forces = {},
	}
	return b
end

-- util

function vec(_x, _y)
	-- default to 0
	xval = _x or 0
	yval = _y or 0
	v = { x = xval, y = yval }
	return v
end

function get_vec_length(v)
	vec_length = sqrt(v.x * v.x + v.y * v.y)
	return vec_length
end

function normalize_vec(v)
	vec_length = get_vec_length(v)
	return vec(v.x / vec_length, v.y / vec_length)
end

function generate_ground()
	for i = 0, 128 do
		ground[pen.x] = pen.y
		pen.x += pen.dx
		pen.y += pen.slope * pen.dx
		pen.slope += -0.2 + rnd(0.4)
		pen.slope = mid(-1, pen.slope, 1)
		-- if btn(2) then
		-- 	pen.slope -= 0.1
		-- end
		-- if btn(3) then
		-- 	pen.slope += 0.1
		-- end
	end
end
